...
@click.option("--ws", type=str, required=True, help="Ethereum WS endpoint. Required to be a GETH client.")
...
def main(rpc, ws, rest_host, rest_port, db_dir):
    database = initialize_database(db_dir)
    loop = asyncio.get_event_loop()
    run_monitor(rpc, ws, database)
    loop.run_until_complete(api.serve(rest_host, rest_port, database))
    loop.run_forever()


def run_monitor(rpc, ws, database):
    chain = Chain(database)
    rpc_client = RPCClient(rpc, ws, chain)
    rpc_client.start()
